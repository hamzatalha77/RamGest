...
<div class="col-12">
			 <div class="box">
				<div class="box-header with-border">
				  <h3 class="box-title">Mes Sorties</h3>
					  ...
					  <table id="example5" class="table table-bordered table-striped" style="width:100%">
						...
						@if(count($sorties))
							...
							<tr>
								...
								<td>{{$sr->fornisseur}}</td>
								<td>{{$sr->client}}</td>
								<td><span class="badge badge-info">{{$sr->prix}} DH</span></td>
								<td><span class="badge badge-success">{{$sr->quantite}} pieces</span></td>
								<td><span class="badge badge-light">{{$sr->total}} DH</span></td>
								<td>{{$sr->created_at}}</td>
								<td>
									<form action="{{ route('sortie.destroy', [$sr->id]) }}" method="POST">
										@csrf
										@method('DELETE')
						...
						<tr id="x_contentStock1" class="x_contentStock1">
                        	<td colspan="10" style="text-align: center">n'a pas de sorties</td>
                    	</tr>
						@endif
						</tbody>
						<tfoot>
