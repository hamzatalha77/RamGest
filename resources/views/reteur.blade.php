...
<div class="col-12">
			 <div class="box">
				<div class="box-header with-border">
				  <h3 class="box-title">Mes Reteurs</h3>
					  ...
					  <table id="example5" class="table table-bordered table-striped" style="width:100%">
						...
						@if(count($reteurs))
							...
							<tr>
								...
								<td>{{$rt->fornisseur}}</td>
								<td>{{$rt->client}}</td>
								<td><span class="badge badge-info">{{$rt->prix}} DH</span></td>
								<td><span class="badge badge-success">{{$rt->quantite}} pieces</span></td>
								<td><span class="badge badge-light">{{$rt->total}} DH</span></td>
								<td>{{$rt->created_at}}</td>
								<td>
									<form action="{{ route('reteur.destroy', [$rt->id]) }}" method="POST">
										@csrf
										@method('DELETE')
						...
						<tr id="x_contentStock1" class="x_contentStock1">
                        	<td colspan="10" style="text-align: center">n'a pas de reteurs</td>
                    	</tr>
						@endif
						</tbody>
						<tfoot>
	  ...
	  <div class="modal-dialog">
		<div class="modal-content">
		  <div class="modal-header">
			<h5 class="modal-title">Ajouter un reteur</h5>
			<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
		  </div>
		  <form action="{{route('reteur.store')}}" method="post">
    	@csrf
